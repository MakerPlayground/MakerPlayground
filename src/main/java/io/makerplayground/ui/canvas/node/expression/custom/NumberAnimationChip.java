...
public class NumberAnimationChip extends Chip<AnimatedValue> {
    ...
    public NumberAnimationChip(AnimatedValue animatedValue) {
        super(animatedValue, Term.Type.NUMBER_ANIMATED);
    }

    @Override
    protected void initView() {
        FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/fxml/canvas/node/usersetting/chip/NumberAnimationChip.fxml"));
    ...
    private void redraw() {
        if (getValue() instanceof ContinuousAnimatedValue) {
            ...
            input.setText(((NumericCategoricalAnimatedValue) getValue()).getKeyValues().stream()
    ...
    @Override
    public Term getTerm() {
        return new NumberAnimationTerm(getValue());
    }
}
